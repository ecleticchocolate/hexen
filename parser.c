...
static void parse_block_body_inside(ASTNode* block) {
    // Mark the alias table. Every alias declared inside this block is popped on exit,
    // so a block-local `alias VT = struct { ... }` is invisible outside it -- and, via
    // alias_lookup's backward scan, shadows any same-named outer alias while inside.
    parse_braced_statements(block, true);
}
...
static void compile_pattern(ASTNode* pat, ASTNode* scrut, Type* scrut_type, ASTNode** out_cond, ASTNode*** out_decls, size_t* decl_count, size_t* decl_cap) {
    ...
    if (pat->type == AST_STRUCT_LITERAL && pat->struct_lit.is_enum_variant) {
        ...
        ASTNode* idxlit = new_node(AST_INT_LITERAL); idxlit->lit_kind = LIT_INT; idxlit->int_value = (uint64_t)vidx;
        ASTNode* eq = new_node(AST_EQ); eq->binary.left = tagread; eq->binary.right = idxlit;
...
static void matchchain_push_stmt(MatchChain* mc, ASTNode* stmt) {
    ASTNode* blk = mc->outer;
    if (blk->block.count >= blk->block.capacity) {
        blk->block.capacity *= 2;
        blk->block.statements = realloc(blk->block.statements, blk->block.capacity * sizeof(ASTNode*));
    }
    blk->block.statements[blk->block.count++] = stmt;
}
...
static ASTNode* parse_match_value(ASTNode* scrut, Type* st) {
    advance(); // '{'

    uint64_t sz = Type_SizeOf(st);
    if (sz == 0) sz = 1;

    bool is_bool = (st->primitive == PRIM_BOOL);
    ...
    static int s_match_ctr = 0;
    char* mname = malloc(32); int mn = snprintf(mname, 32, "$m%d", s_match_ctr++);
    ...
    uint8_t** seen = malloc(8 * sizeof(uint8_t*));
    size_t seen_cap = 8, seen_count = 0;
    bool covered_true = false, covered_false = false;
    bool has_wildcard = false;

    // Enum exhaustiveness (dot-variant syntax through this generic path):
    // track which variants have been matched at the TOP level of a pattern,
    // same idea as the old hardcoded enum path's covered[] array, so `.Some{v}`
    // / `.None` matches can still be checked exhaustive without requiring an
    // `else` arm -- just like the old syntax could.
    ...
    while (s_curr.type != TOK_RBRACE && s_curr.type != TOK_EOF) {
        bool is_wildcard = (s_curr.type == TOK_ELSE);
        ASTNode* pat = NULL;
        ...
        if (!is_wildcard && is_enum_match && pat->type == AST_STRUCT_LITERAL && pat->struct_lit.is_enum_variant) {
            ...
            if (vidx >= 0) {
                ...
                // A variant is FULLY covered only when its payload pattern can't
                // reject any value of that variant -- no payload at all, or a bare
                // identifier bind (matches anything). A refined payload pattern
                // (a literal, or a nested struct/array/enum pattern that can fail)
                // only covers SOME values of the variant, so later arms for the
                // same variant (a catch-all, or a different refinement) are still
                // legal and often necessary for exhaustiveness -- exactly the same
                // "arms tried in order, first match wins" semantics every other
                // match kind here already has.
        ...
        advance();
        SymbolTable* prev_table_arm = s_symtable;
        s_symtable = SymTable_Create(prev_table_arm);
        s_symtable->is_function_scope = prev_table_arm->is_function_scope;
        ...
        if (!is_wildcard) {
            ...
            if (ConstEval_Bytes(pat, bytes, sz)) {
                ...
                if (seen_count >= seen_cap) { seen_cap *= 2; seen = realloc(seen, seen_cap * sizeof(uint8_t*)); }
                seen[seen_count++] = bytes;
        ...
        ASTNode* armblk = new_node(AST_BLOCK);
        armblk->block.capacity = decl_count > 16 ? decl_count : 16;
        armblk->block.statements = malloc(armblk->block.capacity * sizeof(ASTNode*));
        armblk->block.count = 0;

        for(size_t i = 0; i < decl_count; i++) {
            armblk->block.statements[armblk->block.count++] = decls[i];
        }
        if (decls) free(decls);

        parse_block_body_inside(armblk);
        s_symtable = prev_table_arm;
        ...
        if (!cond) {
            cond = new_node(AST_INT_LITERAL);
            cond->lit_kind = LIT_BOOL; cond->int_value = 1;
        }

        ASTNode* ifn = new_node(AST_IF);
        ifn->if_stmt.condition = cond; ifn->if_stmt.true_block = armblk; ifn->if_stmt.false_block = NULL;
    ...
    if (!has_wildcard) {
        ...
        if (is_enum_match) {
            exhaustive = true;
            for (size_t i = 0; i < enum_sd->field_count; i++) {
                if (!enum_covered[i]) { exhaustive = false; break; }
            }
        }
        if (!exhaustive) {
            ...
            if (is_enum_match) {
                ...
                snprintf(errbuf, sizeof(errbuf),
                        "match on %s is not exhaustive -- add an `else` arm "
                        "or handle every variant", enum_sd->name);
                parse_error(errbuf);
            } else if (st->cls == TYPE_PRIMITIVE && !is_bool) {
                parse_error("match on this type is not exhaustive -- add an `else` arm "
                                "(only bool can be exhaustive without one)");
            } else {
                parse_error("match on a struct/array/pointer/function value is not "
                                "exhaustive -- add an `else` arm (these can't be enumerated)");
            }
        }
        // Exhaustive with no `else`: the final arm still carries a condition
        // (`else if (tag == 2) { C }`), so it looks skippable to any structural pass even
        // though every value reaches it. Mark it, so "does every path return?" can tell
        // an exhaustive `match` apart from a bare `if` with no `else`. Codegen ignores
        // this and still emits the compare -- the flag records a fact, it does not change
        // what runs.
...
static ASTNode* parse_match(void) {
    ...
    // TYPE-scrutinee fast path for GROUPED types. A bare type scrutinee
    // (`match u32`, `match Foo*`) already resolves through parse_expr_prec's
    // curr_begins_type branch below. But a scrutinee that *starts with `(`*
    // (a grouped type like `(fn() T)[4]`) would send the expression parser down
    // the parenthesized-expression path and choke on the type content. So: if the
    // scrutinee begins with `(`, try parse_type first with soft-fail — if it
    // yields a type AND the next token is `{` (the arm block), it's a
    // type-scrutinee match; otherwise restore and fall back untouched.
    if (s_curr.type == TOK_LPAREN) {
        LexerState msave;
        Lexer_Save(&msave);
    ...
    // TYPE-scrutinee match: `match T { P* {..} E[N] {..} u32 {..} else {..} }`.
    // The scrutinee parsed as a bare type (AST_TYPE_EXPR) rather than a value —
    // this is reflection over the shape of a type, not a value match. It lowers
    // to a type if-chain (AST_EQ over AST_TYPE_EXPR) whose arms structurally
    // destructure the type, exactly parallel to the enum path below but comparing
    // TYPES instead of reading a tag. Handled entirely in its own function so the
    // value paths stay untouched.
    if (scrut->type == AST_TYPE_EXPR) {
    ...
    // Floats are deliberately excluded from the primitive path: `==` on floats
    // doesn't mean what match's duplicate-arm/exhaustiveness checks assume it
    // means (NaN != NaN; two "equal" values can differ after rounding). A match
    // arm claiming to cover "this value" is a promise a float can't keep -- cast
    // to an int type first if you need this. This is match-specific: unpack has
    // no exhaustiveness/duplicate-arm reasoning to protect, so it never needed
    // this exclusion.
    bool is_matchable_primitive = k.is_primitive && !Type_IsFloat(st);
    bool is_matchable_aggregate = k.is_aggregate;
    bool is_matchable_ptr_or_fn = k.is_ptr_or_fn;

    if (!is_enum && !is_matchable_primitive && !is_matchable_aggregate && !is_matchable_ptr_or_fn)
        parse_error("match scrutinee must be an enum value, a non-float primitive, or a struct/array"
                     " (or a pointer/function value)");

    if (!is_enum) return parse_match_value(scrut, st);
...
static ASTNode* parse_unpack(void) {
    ...
    #define UPUSH(s) do { if (outer->block.count >= outer->block.capacity) { \
        outer->block.capacity *= 2; outer->block.statements = realloc(outer->block.statements, \
        outer->block.capacity * sizeof(ASTNode*)); } \
        outer->block.statements[outer->block.count++] = (s); } while(0)
    ...
    for (size_t i = 0; i < decl_count; i++) UPUSH(decls[i]);
    if (decls) free(decls);
    #undef UPUSH
...
static ASTNode* parse_top_level(void) {
            ...
            if (decl) {
                if (block->block.count >= block->block.capacity) {
                    ...
                    block->block.statements = (ASTNode**)realloc(
                        block->block.statements, block->block.capacity * sizeof(ASTNode*));
                }
    ...
    if (s_curr.type == TOK_IMPL) {
        ...
        while (s_curr.type != TOK_RBRACE && s_curr.type != TOK_EOF) {
            ...
            advance(); // consume 'fn'
            if (s_curr.type != TOK_IDENTIFIER) parse_error("Expected method name");

            // Mangle: Foo_method
            size_t mname_len = impl_type_len + 1 + s_curr.length;
            char* mname = (char*)malloc(mname_len + 1);
            memcpy(mname, impl_type, impl_type_len);
            mname[impl_type_len] = '_';
            memcpy(mname + impl_type_len + 1, s_curr.start, s_curr.length);
            mname[mname_len] = '\0';

            // Temporarily replace the method token with the mangled name so the
            // normal fn parser registers the right symbol. We do this by injecting
            // a fake identifier token and then calling the fn parse path directly.
            Token method_tok = s_curr; // save original (for error msgs)
            (void)method_tok;
            advance(); // consume method name — now s_curr = '[' or '('

            // Build self param type: Foo*
            Type* self_base = (Type*)calloc(1, sizeof(Type));
            self_base->cls = TYPE_STRUCT;
            self_base->struct_name = impl_type; // stable (strndup'd above)

            Type* self_type = (Type*)calloc(1, sizeof(Type));
            self_type->cls = TYPE_POINTER;
            self_type->pointer_base = self_base;

            // Type params for this method = struct's type params (already in s_type_params
            // from the impl-block scope). Methods may also declare extra [U] params;
            // merge them. tparams is what gets stored on the func_decl.
            size_t struct_tparam_count = impl_sd ? impl_sd->type_param_count : 0;
            const char** tparams = NULL;
            Type**       pkinds  = NULL;
            size_t tparam_count = 0;
            // Start with struct params (names + kinds)
            if (struct_tparam_count > 0) {
                tparams = (const char**)malloc(struct_tparam_count * sizeof(char*));
                pkinds  = (Type**)malloc(struct_tparam_count * sizeof(Type*));
                for (size_t i = 0; i < struct_tparam_count; i++) {
                    tparams[i] = impl_sd->type_params[i];
                    pkinds[i]  = impl_sd->param_kinds ? impl_sd->param_kinds[i] : NULL;
                }
                tparam_count = struct_tparam_count;
            }
            // Append any extra method-level [U, u32 K] params (type or value)
            if (s_curr.type == TOK_LBRACKET) {
                const char** extra_names = NULL;
                Type** extra_kinds = NULL;
                size_t extra_count = 0;
                parse_generic_param_list_with_prefix(tparams, pkinds, tparam_count,
                                                    &extra_names, &extra_kinds, &extra_count);
                tparams = extra_names;
                pkinds = extra_kinds;
                tparam_count = extra_count;
                s_type_params = tparams;
                s_param_kinds = pkinds;
                s_type_param_count = tparam_count;
            }

            if (s_curr.type != TOK_LPAREN) parse_error("Expected '(' in impl method");
            advance(); // consume '('

            ASTNode* node = new_node(AST_FUNC_DECL);
            node->func_decl.name = mname;
            node->func_decl.name_len = mname_len;
            ...
            // Inject self as first param (name "self")
            ...
            if (s_curr.type != TOK_RPAREN) {
                while (1) {
                    ...
                    if (!ptype) parse_error("Expected parameter type in impl method");
                    if (s_curr.type != TOK_IDENTIFIER) parse_error("Expected parameter name in impl method");
                    Token pname = s_curr;
                    advance();
                    Symbol* psym = SymTable_Add(s_symtable, pname.start, pname.length, ptype, SYM_LOCAL);
                    node->func_decl.param_syms[node->func_decl.param_count++] = psym;
                    if (s_curr.type == TOK_COMMA) advance();
                    else break;
                }
            }
            if (s_curr.type != TOK_RPAREN) parse_error("Expected ')' in impl method");
            advance(); // consume ')'

            // Return type: present iff next token is not '{'
            Type* ret_type = NULL;
            if (s_curr.type != TOK_LBRACE) {
                ret_type = parse_type();
                if (!ret_type) parse_error("Expected return type or '{' in impl method");
            }
            ...
            ftype->function.is_vararg = false;

            Symbol* func_sym = SymTable_Add(prev_table, mname, mname_len, ftype, SYM_FUNCTION);
            node->func_decl.sym = func_sym;
            if (is_pub) func_sym->is_pub = true;
            func_sym->has_init = true;
            func_sym->func_decl = node;
            ...
            if (block->block.count >= block->block.capacity) {
                block->block.capacity *= 2;
                block->block.statements = (ASTNode**)realloc(block->block.statements, block->block.capacity * sizeof(ASTNode*));
            }
            block->block.statements[block->block.count++] = node;
        }
    ...
    { ASTNode* r = parse_fn_decl(is_pub, is_extern); if (r) return r; }
}
...
static ASTNode* parse_fn_decl(bool is_pub, bool is_extern) {
    if (s_curr.type == TOK_FN) {
        ...
        ASTNode* node = new_node(AST_FUNC_DECL);
        node->func_decl.name = name.start;
        node->func_decl.name_len = name.length;
        node->func_decl.param_count = 0;
        node->func_decl.pack_param_index = -1;
        size_t param_cap = 6;
        node->func_decl.param_syms = (Symbol**)calloc(param_cap, sizeof(Symbol*));
        ...
        Symbol* func_sym = SymTable_Add(prev_table, name.start, name.length, ftype, SYM_FUNCTION);
        node->func_decl.sym = func_sym;
